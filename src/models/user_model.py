...
class UserModel:
    ...
    def dict_to_model(dictionary):
        user = UserModel(dictionary["id"], dictionary["first_name"], dictionary["last_name"], dictionary["email"], dictionary["password"], dictionary["role_id"])
        return user
    ...
    def dicts_to_models(list):
        users = []
        for item in list:
            user = UserModel.dict_to_model(item)
            users.append(user)
        return users